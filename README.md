...
Repo AI is a small AI-powered CLI for understanding and reviewing code repositories.

It is intentionally compact enough for a resume project, while still showing the core ideas behind modern AI developer tools:
...
Repo AI works without network access by using local retrieval and heuristics. To enable generated answers and deeper reviews, configure an OpenAI-compatible API.
...
`repo-ai review` and `repo-ai commit` read `git diff`. With an API key, they use the model. Without one, they fall back to simple but useful local heuristics.

## Resume Description

> Built Repo AI, a Python CLI that indexes local code repositories and provides RAG-based code Q&A, file explanation, Git diff review, and Conventional Commit generation. Designed a lightweight hybrid retrieval pipeline with Chinese query expansion, project overview fallback, line-level citations, and optional OpenAI-compatible model integration, demonstrating practical understanding of AI coding assistants, tool use, and developer workflow automation.
